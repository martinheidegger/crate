...
public class AlterTableAddColumnAnalyzerTest extends CrateUnitTest {
    ...
    public void testAddColumnToATableWithoutPrimaryKey() throws Exception {
        AddColumnAnalyzedStatement analysis = (AddColumnAnalyzedStatement)e.analyze(
    ...
    public void testAddArrayColumn() throws Exception {
        AddColumnAnalyzedStatement analysis = (AddColumnAnalyzedStatement)e.analyze("alter table users add newtags array(string)");
    ...
    public void testAddNewNestedColumnToObjectColumn() throws Exception {
        AddColumnAnalyzedStatement analysis = (AddColumnAnalyzedStatement)e.analyze("alter table users add column details['foo'] object as (score float, name string)");
    ...
    public void testAddNewNestedColumnWithArrayToRoot() throws Exception {
        AddColumnAnalyzedStatement analysis = (AddColumnAnalyzedStatement)e.analyze("alter table users add column new_obj_col object as (a array(long))");
        List<AnalyzedColumnDefinition> columns = analysis.analyzedTableElements().columns();
        assertThat(columns.size(), is(2)); // second one is primary key
    ...
    public void testAddNewNestedColumnWithArrayToObjectColumn() throws Exception {
        AddColumnAnalyzedStatement analysis = (AddColumnAnalyzedStatement)e.analyze("alter table users add column new_obj_col object as (o object as (b array(long)))");
        List<AnalyzedColumnDefinition> columns = analysis.analyzedTableElements().columns();
        assertThat(columns.size(), is(2)); // second one is primary key
    ...
    public void testAddNewNestedColumnToNestedObjectColumn() throws Exception {
        AddColumnAnalyzedStatement analysis = (AddColumnAnalyzedStatement)e.analyze("alter table deeply_nested add column details['stuff']['foo'] object as (score float, price string)");
        List<AnalyzedColumnDefinition> columns = analysis.analyzedTableElements().columns();
